...
void Geometry::uploadData(const float* positions, const float* uvs, const float* normals,
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(uint32_t) * _nVerts, indices, GL_STATIC_DRAW);

    glBindBuffer(GL_ARRAY_BUFFER, _VBO[1]);                 //positions
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * _nVerts * 3, positions, GL_STATIC_DRAW);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void*)0);
    glEnableVertexAttribArray(0);

    glBindBuffer(GL_ARRAY_BUFFER, _VBO[2]);                 //uvs
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * _nVerts * 2, uvs, GL_STATIC_DRAW);
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 0, (void*)0);
    glEnableVertexAttribArray(1);

    glBindBuffer(GL_ARRAY_BUFFER, _VBO[3]);                 //normals
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * _nVerts * 3, normals, GL_STATIC_DRAW);
