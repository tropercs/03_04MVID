...
void handleInput() {
    ...
    for (auto& key : keys) {
        if (key.first == 81) {
            std::cout << "interpolacion linear" << std::endl;
            lin = true;
        }
        if (key.first == 87) {
            std::cout << "interpolacion nearest" << std::endl;
            lin = false;
        }
    }
}
...
uint32_t createTexture(const char* path, bool linear) {
    uint32_t texture;
...
int main(int, char* []) {
    ...
    uint32_t tex1 = createTexture("../assets/textures/stellaris.png", true);
    uint32_t tex2 = createTexture("../assets/textures/stellaris.png", false);
    lin = true;
